...
class Wallet:
    def __init__(self, node_id): #node_id for developpment only. Remove for production
    ...
    def save_keys(self):
        if self.public_key != None and self.private_key != None:
            try:
                with open('wallet-{}.txt'.format(self.node_id), mode='w') as f: #node_id for developpment only. Remove for production
    ...
    def load_keys(self):
        try:
            with open('wallet-{}.txt'.format(self.node_id), mode='r') as f: #node_id for developpment only. Remove for production
                keys = f.readlines()
                public_key = keys[0][:-1]
                private_key = keys[1]
                self.public_key = public_key
